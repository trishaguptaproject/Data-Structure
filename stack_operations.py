...
def stack_operations():
    ...
    while True:
        ...
        if choice == 1:
            ...
            try:
                position = int(input(colored("Enter position (0-based): ", "green")))
                stack.insert(item, position)
            except ValueError:
                cprint("Position must be an integer.", "red")
            except IndexError as e:
                cprint(e, "red")

        elif choice == 2:
            try:
                position = int(input(colored("Enter position to delete: ", "green")))
